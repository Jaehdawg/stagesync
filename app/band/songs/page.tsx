    ...
    <main className="min-h-screen bg-slate-950 px-4 py-8 text-slate-100 sm:px-6 lg:px-8">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-6">
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-3 max-w-2xl text-slate-300">
                Manage the band’s song list, import from CSV, Google Sheets, or Tidal, and keep the singer picker fast.
              </p>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          ...
          <form className="mt-6 grid gap-4 lg:grid-cols-[1.2fr_1.2fr_0.6fr_auto]" action="/api/band/songs" method="post">
            <div className="space-y-2">
              <label htmlFor="song-title" className="text-sm font-medium text-slate-200">Song title</label>
              <input id="song-title" name="title" type="text" className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" placeholder="Dreams" />
            </div>
            <div className="space-y-2">
              <label htmlFor="song-artist" className="text-sm font-medium text-slate-200">Artist</label>
              <input id="song-artist" name="artist" type="text" className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" placeholder="Fleetwood Mac" />
            </div>
            <div className="space-y-2">
              <label htmlFor="song-duration" className="text-sm font-medium text-slate-200">Duration (ms)</label>
              <input id="song-duration" name="durationMs" type="number" min={0} className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" placeholder="300000" />
            </div>
            <div className="flex items-end">
              <button type="submit" className="w-full rounded-xl border border-white/10 bg-white/5 px-4 py-3 font-medium text-white">Save song</button>
            </div>
          </form>
        </section>
        ...
        <section className="rounded-3xl border border-white/10 bg-white/5 p-6">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold text-white">Song list</h2>
              <p className="mt-2 text-sm text-slate-300">{count ?? 0} active songs in the library.</p>
            </div>
            ...
            <div className="divide-y divide-white/10">
              {songs?.length ? songs.map((song) => (
                <div key={song.id} className="grid gap-4 bg-slate-950/50 p-4 lg:grid-cols-[1.2fr_1.2fr_0.6fr_auto] lg:items-center">
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Artist</p>
                    <p className="mt-1 text-base font-semibold text-white">{song.artist}</p>
                    <p className="mt-2 text-xs uppercase tracking-[0.18em] text-slate-500">{song.source_type ?? 'uploaded'}</p>
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Song title</p>
                    <p className="mt-1 text-base font-semibold text-white">{song.title}</p>
                    {song.source_ref ? <p className="mt-2 text-xs text-slate-500">Ref: {song.source_ref}</p> : null}
                  </div>
                  <div>
                    <p className="text-xs uppercase tracking-[0.22em] text-slate-400">Duration</p>
                    <p className="mt-1 text-base font-semibold text-white">{formatDuration(song.duration_ms)}</p>
                  </div>
                  <div className="flex flex-wrap gap-2 lg:justify-end">
                    <AdminRowDialog triggerLabel="Edit" title={`Edit ${song.title}`}>
                      <form className="grid gap-4 rounded-2xl border border-white/10 bg-slate-950/50 p-5" action={`/api/band/songs/${song.id}`} method="post">
                        ...
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-slate-200" htmlFor={`song-title-${song.id}`}>Song title</label>
                          <input id={`song-title-${song.id}`} name="title" defaultValue={song.title} className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-slate-200" htmlFor={`song-artist-${song.id}`}>Artist</label>
                          <input id={`song-artist-${song.id}`} name="artist" defaultValue={song.artist} className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" />
                        </div>
                        <div className="space-y-2">
                          <label className="text-sm font-medium text-slate-200" htmlFor={`song-duration-${song.id}`}>Duration (ms)</label>
                          <input id={`song-duration-${song.id}`} name="durationMs" type="number" min={0} defaultValue={song.duration_ms ?? ''} className="w-full rounded-xl border border-white/10 bg-slate-900/80 px-4 py-3 text-white" />
                        </div>
                    ...
                    <form action={`/api/band/songs/${song.id}`} method="post">
                      <input type="hidden" name="action" value="delete" />
                      <button type="submit" className="rounded-full border border-white/10 px-3 py-1 text-xs text-slate-200">Archive</button>
                    </form>
                  </div>
                </div>
              )) : (
                <div className="bg-slate-950/50 p-6 text-slate-400">No songs yet. Import one of the lists above or add a song manually.</div>
              )}
            </div>
          </div>
          ...
          <div className="mt-6 flex items-center justify-between gap-3">
            <Link
              href={buildHref('/band/songs', { page: currentPage - 1, q })}
              className={`rounded-xl border px-4 py-2 text-sm font-medium ${currentPage <= 1 ? 'pointer-events-none border-white/5 bg-white/5 text-slate-500' : 'border-white/10 bg-white/5 text-white hover:border-cyan-400/50'}`}
            >
              Previous
            </Link>
            <p className="text-sm text-slate-400">Page {currentPage} of {totalPages}</p>
            <Link
              href={buildHref('/band/songs', { page: currentPage + 1 > totalPages ? totalPages : currentPage + 1, q })}
              className={`rounded-xl border px-4 py-2 text-sm font-medium ${currentPage >= totalPages ? 'pointer-events-none border-white/5 bg-white/5 text-slate-500' : 'border-white/10 bg-white/5 text-white hover:border-cyan-400/50'}`}
            >
              Next
            </Link>
          </div>
