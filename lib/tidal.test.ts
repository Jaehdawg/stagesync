...
describe('tidal helpers', () => {
  ...
  it('filters album hits out of catalog search results', async () => {
    const originalClientId = process.env.TIDAL_CLIENT_ID
    const originalClientSecret = process.env.TIDAL_CLIENT_SECRET
    delete process.env.TIDAL_BROWSER_TOKEN
    process.env.TIDAL_CLIENT_ID = 'client-id'
          ...
          json: async () => ({
            data: {
              relationships: {
                directHits: {
                  data: [
                    { id: '121444600', type: 'tracks' },
                    { id: '44732491', type: 'albums' },
                  ],
                },
              },
            },
            included: [
              {
                id: '121444600',
                type: 'tracks',
                attributes: { title: 'Shake It Off', duration: 'PT3M39S' },
                relationships: { artists: { data: [{ id: '3557299', type: 'artists' }] } },
              },
              {
                id: '44732491',
                type: 'albums',
                attributes: { title: 'Shake It Off (Taylor Swift Covers)' },
                relationships: { artists: { links: { self: '/albums/44732491/relationships/artists?countryCode=US' } } },
              },
            ],
          }),
        } as Response
